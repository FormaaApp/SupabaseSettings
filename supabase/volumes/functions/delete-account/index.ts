...
Deno.serve(async (req) => {
  try {
    ...
    if (userError || !user) {
      // Временно отдаём настоящую причину (а не общее "Unauthorized") —
      // getUser() падает даже с валидным токеном сессии, если SUPABASE_ANON_KEY
      // в окружении функции не совпадает с ключом, которым подписан токен
      // (например, после ротации/перехода на новый формат ключей проекта).
      // Без текста самой ошибки это неотличимо от "юзер правда не залогинен".
      return new Response(
        JSON.stringify({ error: "Unauthorized", detail: userError?.message ?? "no user in response" }),
        { status: 401 }
      )
    }
